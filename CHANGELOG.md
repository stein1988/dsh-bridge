...
- **cloudflared 运行时健康探针**：解析 cloudflared 自报 metrics 地址并每 30s 探活官方 `/ready`（判据与上游一致：HTTP 200 且 `readyConnections > 0`）。连续 3 次失败（约 90s）→ 面板状态可见降级「⚠️ 连接器已无健康边缘连接」；连续 10 次（约 5min）→ 判定假死、终止进程并走既有自愈链路强制重建。重建链路（含重启后握手失败）**永不进入终态**——网络恢复后会自动自愈，不再需要人工干预。
- **cloudflared 输出落盘**：子进程输出写入 `~/.dsh/dsh-bridge/cloudflared.log`，超 5MB 自动轮转为 `.1`；隧道 Token 落盘前强制脱敏（含跨输出块切分、以及 Token 前缀残留等边界，确保任何 Token 片段都不落盘）。

### 🐞 修复

- **公网入口静默失效数天无人发现**（现象：HTTP 530 + `error code: 1033`）：旧逻辑只监听子进程退出，而假死连接器既不退出也不报错，故永不触发自愈。探针上线后最迟约 5 分钟即可自动重建。
- **本机点「添加工作区」误弹插件的远程抽屉**（issue #28 第 2 条）：DSH 0.1.5 起自带官方目录选择器（`dsh-host-directory-picker-auto` 按宿主能力分发 native / browse），且注册到与插件完全相同的 Slot；DSH 的 slots 是 shadow 语义（动态注册的插件天然覆盖内置实现），于是本机 `127.0.0.1:3080` 也被插件的远程目录选择器接管。现改为「本机访问 且 官方 picker 可用」时**不注册**该 Slot，让位给 DSH 官方——macOS / Windows 走系统原生文件夹对话框，无桌面环境的 Linux 走官方网页选择器；**旧版 DSH 与远程 / 移动访问仍由插件接管**，保留管理密码解锁与 `local_only` 目录策略。
- **添加工作区弹窗点过「已注册工作区」后永久卡死**（PR #42）：`switchToWorkspace()` 只置位 `isSubmitting` 而从不复位，同一弹窗实例的 `loadDirectory()` / `doSubmit()` 守卫让弹窗彻底无响应（点目录不进目录、点添加无反应且按钮停在「正在添加并切换…」）；改为与 `doSubmit()` 对称的 `try/finally` 复位，`render()` 抛错与将来新增的提前 return 都不会再泄漏。
- **探针自身异常不得拖垮宿主**：畸形 metrics 地址、日志通道自身抛错等任何探针路径异常都不会变成未处理 rejection 结束 DSH 主进程（独立子进程实验验证）。
- **新增 18 条探针回归测试**（含防误杀正常重连连接器、防终态、脱敏泄漏/吞字符、跨 chunk 脱敏、崩溃回归等）。
- **新增 15 条回归测试**：12 条目录选择器让位（让位判定真值表、官方 picker 探测、Slot 注册顺序结构断言、产物同步）+ 3 条添加工作区弹窗 `isSubmitting` 复位与产物同步。

> ⚠️ 生效说明：本版需要重启一次 dsh（`systemctl --user restart dsh-web.service` 或面板「重启 DSH」）才加载新代码；重启会短暂中断当前会话与公网隧道（数十秒）。
