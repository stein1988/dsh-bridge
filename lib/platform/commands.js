  ...
  switch (command) {
    ...
    case 'rename': {
      ...
      try {
        ...
        // 标题的权威入口是 DSH 的 session-title 服务：它会把 `session/title` 事件
        // append 进会话日志，从而**真正持久化**。旧代码调用
        // `sessionPersistence.update(id, { title })` —— 该 API 在 0.1.5 并不存在
        // （SessionPersistence 只有 create/open/flush/stat/list），且被 .catch 吞掉，
        // 结果是 `/rename` 只改了内存、重启即丢。
        const titles = typeof node.ctx.get === 'function' ? node.ctx.get('sessionTitle') : node.ctx.sessionTitle
        if (session && typeof titles?.rename === 'function') {
