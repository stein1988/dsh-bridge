...
/**
 * 取出 `sessionPersistence.list()` 条目里的会话头（SessionHeader）。
 *
 * DSH 0.1.5 起该方法返回的是**快照** `{ header, revision, sizeBytes }`
 * （见 `dsh-session-persistence-jsonl` 的 `list()`，以及 `dsh-workspace` 的
 * `(await ctx.sessionPersistence.list()).map((snapshot) => snapshot.header)`）；
 * 更早的版本直接返回 header 数组。两种形状都要兼容：只按 `entry.id` 取值的旧写法
 * 在新版上恒为 undefined，会把"已持久化的会话"误判成"未持久化"，进而对已存在的
 * 会话调用 `agents.create`，抛出 `session "..." already exists`（issue #39）。
 *
 * @param {object} entry list() 返回的单个条目
 * @returns {object|undefined} 会话头，形状不可识别时返回 undefined
 */
export function sessionHeaderOf(entry) {
...
// 读取「冷会话」（不在内存里）的事件日志，用于折叠标题。
//
// 这是官方 v2.10.10 没覆盖到的一个缺口：它用 sessionHeaderOf() 修好了 list() 的
// snapshot 形状（issue #39），但折叠标题那一步仍在调用 `sessionPersistence.load()`，
// 而该方法在 DSH 0.1.5 并不存在（真实契约只有 create/open/flush/stat/list），
// 且被 catch{} 吞掉 —— 表现为冷会话标题恒为「新会话」。
//
// 事件内容只能通过 `open(id, 'read')` 拿到句柄后 `handle.read()` 读取，
// 且 snapshot 里不含 events。这里统一走官方 API 补上这一环。
async function readStoredEvents(persistence, sessionId) {
  if (!persistence?.open || !sessionId) return undefined
  let handle
  try {
    handle = await persistence.open(sessionId, 'read')
    const result = await handle?.read?.()
    return Array.isArray(result?.events) ? result.events : undefined
  } catch {
    return undefined
  } finally {
    try {
...
// 把 sessionPersistence.list() 的 snapshot 归一化成扁平形态。
//
// 两种形态都要兼容：
//   - 官方 snapshot：{ header: { id, createdAt, cwd, ... } }
//   - 早期/兜底形态：{ id, createdAt, cwd, ... }（单测与旧后端）
// 注意：归一化本身由官方 v2.10.10 的 sessionHeaderOf() 负责（见上），此处不重复实现。

export function isSubagentSession(cacheRow, liveSession) {  if (liveSession?.origin === 'subagent' || liveSession?.header?.origin === 'subagent') return true
...
export async function listSessions(node) {
  ...
  if (workspaces.length > 0) {
    for (const ws of workspaces) {
      ...
      for (const sId of ws.sessionIds) {
        ...
        if (live) {
          ...
          const events = await readStoredEvents(node.ctx.sessionPersistence, sId)
          title = foldTitle(events) ?? undefined
        }
    ...
    if (node.ctx.sessionPersistence?.list) {
      try {
        const entries = await node.ctx.sessionPersistence.list()
        if (Array.isArray(entries)) {
          // 兼容 DSH 0.1.5 的 { header, revision, sizeBytes } 快照与更早的扁平 header 数组。
          // 旧写法按 entry.id 取值，在新版上恒为 undefined，会把「已持久化的会话」误判成
          // 「未持久化」（issue #39），因此必须经 sessionHeaderOf() 归一化。
          const coldHeaders = entries.map(sessionHeaderOf)
            .filter((h) => h && !accounted.has(h.id) && !archived.has(h.id) && !h.archived)
          for (const h of coldHeaders) {
            accounted.add(h.id)
            // 标题只能从事件日志折叠：snapshot 里没有 events，且 SessionPersistence
            // 不提供 load()（真实方法只有 create/open/flush/stat/list），
            // 必须走 open(id,'read') → handle.read() 读取。
            const events = await readStoredEvents(node.ctx.sessionPersistence, h.id)
            result.push({
              id: h.id,
