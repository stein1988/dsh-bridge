...
//
// DSH 0.1.5 的 SessionPersistence 契约是 create/open/flush/stat/list —— **没有** `load()`：
//   - 事件内容只能通过 `open(id, 'read')` 拿到句柄后 `handle.read()` 读取；
//   - `flush()`/`list()` 返回的是 snapshot（{ header, revision, ... }），**不含 events**。
// 这里必须走官方 API，否则会静默拿不到标题（旧代码调用不存在的 `.load()`，
// 又被 catch 吞掉，表现为 IM 里所有冷会话都显示成「新会话」）。
...
function normalizeStoredEntry(entry) {
  ...
  const header = entry.header && typeof entry.header === 'object' ? entry.header : entry
  const id = header.id
  if (!id) return null
  return {
    id,
    ...
    cwd: header.cwd,
    archived: header.archived === true || entry.archived === true,
  }
}
...
export async function listSessions(node) {
  ...
  if (workspaces.length > 0) {
    ...
    if (node.ctx.sessionPersistence?.list) {
      try {
        const snapshots = await node.ctx.sessionPersistence.list()
        if (Array.isArray(snapshots)) {
          const coldEntries = snapshots
            .map(normalizeStoredEntry)
            .filter((entry) => entry && !accounted.has(entry.id) && !archived.has(entry.id) && !entry.archived)
          for (const entry of coldEntries) {
            accounted.add(entry.id)
            const events = await readStoredEvents(node.ctx.sessionPersistence, entry.id)
            result.push({
              id: entry.id,
